...
(ROC) analysis, is widely used for this purpose. ROC studies are often
required for regulatory approval of new devices. The purpose of this
work is to describe \pkg{RJafroc}, which implements software for
...
Windows software called JAFROC (jackknife alternative free-response
ROC, V4.2.1, \url{http://www.devchakraborty.com}). In the ROC paradigm
the radiologist rates each image for confidence in presence of disease.
The images are typically split equally between actually non-diseased
and diseased. A common figure of merit (FOM) is the area under the
ROC curve, which has the physical interpretation as the probability
that a diseased image is rated higher than a non-diseased one. In
ROC studies a number of radiologists (readers) rate images in two
or more treatments, and the object of the analysis is to determine
the significances of the inter-treatment differences between reader-averaged
FOMs. In the free-response (FROC) paradigm the reader marks the locations
...
in widespread use, a jackknife pseudovalue based model due to Dorfman-Berbaum-Metz
(DBM) and an ANOVA model with correlated errors due to Obuchowski-Rockette
(OR), both of which have been improved by Hillis. Included are tools
...
of statistics that is of great importance in medicine, where new imaging
technology and the accuracy of interpretations often need to be assessed.
...
There are, conservatively, at least 1000 publications describing ROC
...
can be very expensive to conduct. For example \citep{PisGatDiagnPerf2005}, the Digital
...
for analyzing ROC studies and its extensions, and four websites disseminate
...
JAFROC (jackknife alternative free-response ROC, V4.2.1), which can analyze ROC studies and 
...
has been used in 77 publications: the list is viewable at
\url{http://www.devchakraborty.com/JafrocApplications.pdf}.
The purpose of this work is to describe a package called \pkg{RJafroc},
which is an enhanced implementation of JAFROC. In this section we introduce terminology
used in the \code{RJafroc-package} page of the documentation that
accompanies this paper. Several reviews of this field may be consulted
for details \citep{MetzBasPrin1978,MetzROCMeth1986,MetzSomePra1989,WagnBeidAsseOfMedi2002,WagnMetzAssOfMedi2007,KunBerRece2008,MetzROCAna2008} regarding basics of ROC methodology. [An
existing package \pkg{ROCR} \citep{SingSandROCRVisu2005} for classifier performance
evaluation and visualization, while useful in the machine learning,
pattern recognition and artificial intelligence fields, is not suitable
for the medical imaging applications addressed in the various software
...
of disease. The rating \emph{r} is typically on a numeric scale, with
higher values representing increasing confidence in presence of disease
and lower values representing increasing confidence in absence of
disease. Typically 5 or 6 integer ratings are used but the ratings
could have higher precision. With a 6 rating scale a 1-rating would
...
fraction} $FPF_{3+}$, where $FPF$ is the abscissa. It can be seen
...
operating points, belonging to any dataset, obtained by counting
...
\emph{figure of merit} (FOM) for quantifying observer performance \citep{HanMcNTheMean1982}.
It can be shown to be equivalent to the Mann-Whitney-Wilcoxon 2-sample
U-statistic \citep{WilcoxonIndiComp1945,MannWhitOnATest1947}. True positive fraction is synonymous with
...
$sensitivity$ vs. $1-specificity$. ROC studies are typically conducted
...
or computed tomography (CT) scans. Ignoring localization can result in an overestimate 
...
with the disease-free region. There are two data collection paradigms
...
implemented in any of the websites mentioned so far). In the \emph{free-response}
paradigm \citep{EgaGreOperat1961,MillerTheFROC1969,BunchHamAFree1978} the radiologist marks and rates regions that are
...
positive in the FROC, LROC or ROI contexts can lead to confusion). Unmarked lesions
...
inaccessible to the observer, it needs to be taken into account in
defining the area under the AFROC as a valid figure of merit \citep{ChakraASear2006,ChakraROC2006}; 
...
By treating the rating of the highest rated mark on a diseased image (or negative infinity if
...
others and therefore more important to find). With the exception of
the FROC, the trapezoidal areas under all of these curves qualify
as valid figures of merit (a valid figure of merit is one that rewards
...
FROC is a particularly bad figure of merit can be appreciated from
...
is divided into Q regions of interest (typically Q is 4 or 5) where
each region is either non-diseased or diseased, and the reader gives
a ROC-like rating to each region. Regarding each of the regions as
...
- reader combination, of the figure of merit. One object of the analysis
...
are applicable to any scalar figure of merit}. In fact current JAFROC
software uses the DBMH significance testing method and applies it
to different figures of merit, e.g., the trapezoidal area under the
AFROC curve.


\begin{table}[htbp]
  \centering
  \caption{Data collection paradigms, associated operating characteristics,
figures of merit and common terminology}
    ...
    \midrule
    Receiver operating characteristic & ROC = TPF vs. FPF & Trapezoidal area under ROC & AUC \\ \hline
...
\begin{table}[htbp]
  ...
  \caption{Software availability of MRMC observer performance methods.}
    ...
    \midrule
    DBMH, ORH & OR-DBM MRMC / \url{http://perception.radiology.uiowa.edu} &  & Wilcoxon and parametric fits \\ \cline{1-2} \cline{4-4}
    DBMH  & Metz ROC Software / \url{http://metz-roc.uchicago.edu/MetzROC/software} &    ROC   & Wilcoxon and parametric fits \\ \cline{1-2} \cline{4-4}
    Mechanistic MRMC & iMRMC / \url{https://code.google.com/p/imrmc} &       & Wilcoxon \\ \hline
    DBMH  & JAFROC / \url{http://devchakraborty.com} & ROC, FROC, ROI & Trapezoidal areas under ROC, AFROC, AFROC1, weighted versions and ROC', and other FOMs \\ \hline
    ...
    Hierarchical ordinal regression\citep{IshGatAGener2000,ObuBeiMultiMulti2004} &   NA    &   ROC    &  AUC and other ROC figures of merit  \\ \cline{1-1}
...
a new \emph{pivotal} study that is sufficiently powered to detect
a clinically relevant difference between two modalities of interest.
The pilot study is used to get estimates of variability components
entering a figure of merit model, as these determine the sample size.
Sample-size estimation methods for ROC studies are available on all
...
The figure of merit is a critical determinant of statistical power \citep{ChakraVali2008}
...
commonly used in analyzing free-response data (several other implemented free-response figures of merit are defined in Appendix \ref{subsec:otherfroc}), 
followed by the ROI figure of merit. Two implemented significance-testing methods are described followed by sample-size estimation for ROC studies.
No derivations are given: we simply refer the journal reader to the
appropriate literature.

\subsection{Figure of merit for ROC data}
...
cases and $K_{2}$ is the number of diseased cases. Let ${z_{ij{k_{t}}t}}$
denote the rating given to case ${k_{t}}t$ by the reader $j$
using modality $i$ with $i=1,2\ldots,I$ and $j=1,2\ldots,J$,
where $I$ is the number of modalities and $J$ is the
...
\begin{equation}
\widehat{\theta}_{ij}=\frac{1}{{{K_{1}}{K_{2}}}}\sum\limits _{{k_{1}}}^{{K_{1}}}{\sum\limits _{{k_{2}}}^{{K_{2}}}{\psi\left({{z_{ij{k_{1}}1}},{z_{ij{k_{2}}2}}}\right)}}\label{eq:1}
\end{equation}
...
\psi\left({{z_{ij{k_{1}}1}},{z_{ij{k_{2}}2}}}\right)=1 &  & {z_{ij{k_{1}}1}}<{z_{ij{k_{2}}2}}\\
\psi\left({{z_{ij{k_{1}}1}},{z_{ij{k_{2}}2}}}\right)=0.5 &  & {z_{ij{k_{1}}1}}={z_{ij{k_{2}}2}}\\
\psi\left({{z_{ij{k_{1}}1}},{z_{ij{k_{2}}2}}}\right)=0 &  & {z_{ij{k_{1}}1}}>{z_{ij{k_{2}}2}}
\end{array}\right\} \label{eq:2}
\end{equation}


This figure of merit can be shown to be identical to the area under
...
\subsection{Figures of merit for free-response data}
...
The notation is driven by the Chakraborty \emph{search-model} for
...
sites}) are identified (based on eye-tracking measurements this phase
...
data. The highest rating inferred ROC (IR) figure of merit $\theta_{ij}^{IR}$
is estimated by (this is identical to the A0 figure of merit defined
...
asterisks. For the second max function, the maximum over diseased
cases, the maximum is over all marks (NLs and LLs), so on a diseased
case there is a possibility that a non-lesion localization is rated
higher than any lesion localization on that case. If all lesions are
...
which is the worst possible ROC performance. Therefore, $\widehat{\theta}_{ij}^{IR}$
ranges between 0.5 and unity. The Song et al \citep{SongBandOnComp2008} A1 figure of
merit takes the average rating of all marked regions on an image to
infer an ROC-like single rating for the image. The Song A2 
figure of merit involves a stochastic dominance idea.

Let ${W_{{k_{2}}{l_{2}}}}$ denote the weight of lesion ${l_{2}}2$
...
The weighted (according to clinical importance) JAFROC figure of merit $\theta_{ij}^{wJAFROC}$ is estimated
by:
...
$\psi$ function is unity and $\widehat{\theta}_{ij}^{wJAFROC}$ is unity, the best possible performance. 
If no lesions are marked and every
...
0 and unity, unlike the ROC area figure of merit that ranges between 0.5
and 1. The above figure of merit does not count NLs on diseased cases.
The extension to include the highest rated NL on diseased cases, called
the weighted JAFROC1 figure of merit, $\widehat{\theta}_{ij}^{wJAFROC1}$, is:
...
The above two figures of merit have covered the needs of most
users of JAFROC. Other implemented free-response figures of merit,
sometimes needed for specific clinical reasons, are described in 
Appendix \ref{subsec:otherfroc}.

\subsection{Figure of merit for ROI data}
...
instead uses resampling methods for signficance testing. 
Let ${z_{ij{k_{2}}2{l_{2}}2}}$ denote the
...
case. Similarly, let ${z_{ij{k_{t}}t{l_{1}}1}}$ denote the rating
...
\begin{equation}
\widehat{\theta}_{ij}^{ROI}=\frac{{\sum\limits _{{k_{1}}=1}^{{K_{1}}}{\sum\limits _{{k_{2}}=1}^{{K_{2}}}{\sum\limits _{t=1}^{2}{\sum\limits _{{l_{1}}=1}^{{q_{{k_{t}}t1}}}{\sum\limits _{{l_{2}}=1}^{{q_{{k_{2}}22}}}{\psi\left({{z_{ij{k_{t}}t{l_{1}}1}},{z_{ij{k_{2}}2{l_{2}}2}}}\right)}}}}}}}{{\sum\limits _{{k_{1}}=1}^{{K_{1}}}{\sum\limits _{{k_{2}}=1}^{{K_{2}}}{\sum\limits _{t=1}^{2}{\sum\limits _{{l_{1}}=1}^{{q_{{k_{t}}t1}}}{\sum\limits _{{l_{2}}=1}^{{q_{{k_{2}}22}}}{\left(1\right)}}}}}}}\label{eq:7}
\end{equation}
...
ROIs on diseased cases. Unlike the ROC figure of merit and the weighted
JAFROC figure of merit, the ROI figure of merit, like the weighted
JAFROC1 figure of merit, can be defined over a dataset with no non-diseased
cases. Table~\ref{tab:sumfom} summarizes the figures of merit described so far.

\begin{table}[htbp]
  \centering
  \caption{Summary of the figures of merit for the different observer
performance measurement data collection methods. [IR = inferred
ROC using the highest rating; A1, A2 are inferred ROC figures of merit]}
...
possible figures of merit, \emph{it is understood that all calculations
...
Berbaum, Metz, Hillis and others. Only the final results are summarized
here. The starting point is calculation of the mean squares. In the
following definitions the Y subscript emphasizes that the relevant
...
Hillis proposes the following statistic for testing the null hypothesis
...
and the Wilcoxon FOM (the bootstrap and the jackknife are more generally
applicable to any figure of merit).
...
According to Hillis, the observed statistic $F_{ORH}^{*}$ is distributed
as an F-statistic with {$ndf=I-1$} and $ddf_{ORH}$ degrees of
freedom:

\begin{equation}
F_{ORH}^{*}\sim{F_{ndf,ddf_{ORH}}}\label{eq:29}
\end{equation}
...
\begin{equation}
ddf_{ORH}={\frac{{\left[{MS\left({TR}\right)+\left({J\left({\widehat{Cov_{2}}-\widehat{Cov_{3}}}\right)}\right)}\right]}}{{{\frac{{\left[{MS\left({TR}\right)}\right]}}{{\left({I-1}\right)\left({J-1}\right)}}}^{2}}}^{2}}\label{eq:ddfhor}
\end{equation}


For the Wilcoxon statistic, the two definitions of $ddf_{H}$ (Eqn. 
\ref{eq:ddfhdbm} and Eqn. \ref{eq:ddfhor}) are equivalent. The critical
value of the F-statistic for rejection of the null hypothesis is given
by ${F_{1-\alpha,ndf,ddf_{H}}}$. The p-value of the test is given
by:

\begin{equation}
p=P\left({F>F_{ORH}^{*}|F\sim{F_{ndf,dd{f_{ORH}}}}}\right)\label{eq:31}
\end{equation}


The percent $\left({1-\alpha}\right)100$confidence interval for $\left({{\widehat\theta_{i}}-{\widehat\theta_{i'}}}\right)$
is given by

\begin{equation}
C{I_{1-\alpha}}=\left({\widehat{{\theta_{i\bullet}}}-\widehat{{\theta_{i'\bullet}}}}\right)\pm{t_{\alpha/2;dd{f_{ORH}}}}\sqrt{\frac{2}{J}\left({MS\left({TR}\right)+J\max\left({Co{v_{2}}-Co{v_{3}},0}\right)}\right)}\label{eq:32}
\end{equation}
...
in figures of merit between the two modalities) is $2\left|{\widehat{{\tau_{1}}}}\right|$.
...
The sample size procedure \citep{HillObuPower2011} assumes random-readers and random
...
area under the ROC curve as the figure of merit, one might choose
the value observed in the pilot study. } 
...
the pilot dataset, and let $J,K$ be the
...
until the target power is reached. The procedure could be repeated
with different values of $J$ (depending
...
and that the package has been loaded using the \code{library()}
function.
...
for details regarding the ROI simulator). Their structures are shown
below:
...
The ROC dataset has two modalities, five readers, 69 (= 114 - 45) non-diseased
...
are special cases of free-response data, the same data structure is
used to accommodate all of them. The \code{dataType} field can
be \code{ROC}, \code{FROC} or \code{ROI}. For a given modality
and reader, for ROC data the FP ratings are addressed by the first ${K_{1}}$ values of the third dimension 
of the NL array and the corresponding
TP ratings are addressed by the ${K_{2}}$ values of the third dimension
of the LL array. The fourth dimension on the NL and LL arrays, only
the first value of which is used to address ROC ratings, corresponds
to the location index ${l_{s}}s$, i.e., the multiple marks of a given
type, NL ($s=1$) or LL ($s=2$), that are possible for FROC data.
In the above example, the dimensioning of the NL array shows that
there is least one image in the dataset with 7 NL marks, while the
dimensioning of the LL array shows that there is at least one diseased
...
has the same dimensions as \code{lesionID}. The variables \code{ModalityID}
and \code{readerID} are string arrays of length $I,J$, respectively,
that are used to identify the modalities and readers, respectively.
...
number of actually diseased ROIs varies from 1 to 4. The simulator
is available from \url{http://www.devchakraborty.com/RoiData/RoiSimulator.zip}.


\subsection{Creating dataset objects}

The user can manually (or using code) create dataset objects by adhering
to the structure described above (this could be useful in running
simulation studies). For single datasets it is more convenient to
...
below. The \code{ReadDataFile()} function reads the data in JAFROC
...
In each case it returns a dataset object. The MRMC and iMRMC formats
apply to ROC data only while the JAFROC format applies to all paradigms.
The JAFROC Excel file contains three worksheets: 
...
and it appears once in either the FP or TP worksheet. Sample data
files are available on the JAFROC website. The following example downloads
each of them and reads them.
...
(readers and/or cases) to regard as random (the default is \code{ALL}).
The return value of the \code{DBMHAnalysis()} is a list of 22 elements.
Both functions use the weighted JAFROC figure of merit as the default,
so to analyze ROC and ROI paradigm data one must explicitly specify
the figure of merit options as shown below. To apply DBMH significance
testing to a ROC dataset object:
...
The returned data object, which can be viewed using the \code{print()} function of \proglang{R}, has the structure shown in Table~\ref{tab:dbmret}. The output
...
by \code{DBMHAnalysis}. [\code{Trt} = treatment, \code{Rdr}
= reader, \code{RRRC} = random reader random case, \code{FRRC}
= fixed reader random case, \code{RRFC} = random reader fixed case,
\code{ci} = $1-\alpha$ confidence interval, \code{fomArray}
= $\widehat{\theta}_{ij}$, \code{f} = observed F-statistic,
\code{p} = p-value, \code{DiffTrt} = reader-averaged FOM
differences between pairs of modalities, \code{ddf} = denominator
degrees of freedom for F-test (the numerator degrees of freedom is
always $I-1$), \code{ AvgRdrEachTrt} = FOM averaged over all readers,
for each treatment, \code{varComp} = the DBM pseudovalue variance
components.]}
    ...
    \midrule
    \code{fomArray} & The figure of merit array of each reader and modality. \\
...
method is selected with a figure of merit that is not the Wilcoxon
statistic. The return value of the \code{ORHAnalysis()}is a list
...
(the University of Iowa Windows software). Since ORH yields similar
results as DBMH (they are identical for the Wilcoxon figure of merit)
...
the true effect size). The following commands performs DBH analysis
and extracts the relevant pseudovalue variance components and effect
size for sample size estimation:
...
(default 0.8) for different specified values of $J$. Since the pilot
...
study designs. The preceding analysis assumed \code{RRRC}; to get results assuming fixed readers, supply the option \code{randomOption = "CASES"}; to get results assuming fixed cases, supply the option \code{randomOption = "READERS"}. 
...
These are identical to those obtained with DBMH analysis. \cite{HillisObuComparison2005} have
shown that when the Wilcoxon is used as the figure of merit, and jackknifing
is used to estimate the covariance matrix, the two methods will yield
identical results for multiple reader studies.
...
merit is a scalar, and well-behaved (rewards good decisions and penalizes
bad decisions) significance testing methods developed for ROC apply
to the selected figure of merit. We illustrate analysis of this dataset
using the function \code{DBMHAnalysis()}, noting that w\code{JAFROC}
is the default figure of merit. There are 100 non-diseased and 100
diseased images, with the number of lesions on the diseased images
ranging from 1 to 3, in this free-response dataset. The two modalities
are labeled 4 and 5 (the full dataset, containing data for 5 modalities,
is available from author DPC).
...
the default figure of merit is that the software is primarily designed
to analyze free-response data (none of the other mentioned websites
have this capability) and by doing weighted analysis each diseased
...
figure of merit option to \code{JAFROC} or \code{JAFROC1}, the
...
The \code{JAFROC1} figures of merit use all highest rated NL marks,
...
and \code{wJAFROC1} figures of merit is that they will depend on
...
effects are accounted for. This issue also applies to the ROI paradigm,
where one can use a dataset with no non-diseased cases. For these
reasons we prefer \code{JAFROC} and particularly \code{wJAFROC}
figures of merit for characterizing free-response performance.
...
readers and random cases are shown. The Song figures of merit, particularly
...
\begin{table}[htbp]
  \centering
  \caption{Results of DBMH-analysis, using inferred ROC figures of merit
(\code{HrAuc}, \code{SongA1} and \code{SongA2}), applied
to a free-response data}
    \begin{tabular}{cccc}
    \toprule
...
figure of merit for each modality is smaller than the corresponding
inferred ROC figures of merit. This is because of the localization
...
if they is correctly localized, while TPF is only concerned with the
inferred single ratings per case. (3) The effect size is larger for
\code{JAFROC} (0.0548) than for any of the inferred ROC methods (about
0.047 for Song A2). Since effect size appears as the square in sample
size calculations, this contributes towards JAFROC's higher statistical
power. The reason for the larger JAFROC effect size is that the figure
of merit has a larger range over which it can vary, 0 to 1, while
any ROC figure of merit is restricted to the range 0.5 to 1.
...
is the default figure of merit, one needs to explicitly specify the
\code{ROI} figure of merit when using the function \code{DBMHAnalysis()}.
...
The function \code{OutputReport} is used to generate
a report closely patterned on that of OR-DBM MRMC and DBM-MRMC. The
following commands illustrate the usage of this function.
...
The \code{dataDscrpt} option is only needed if a dataset object
is specified. It is a string description of the dataset object, the
default being the variable name of the dataset object. One can explicitly
specify the output file name using the \code{reportFile} option,
as in the second example. If this parameter is missing, the function
will use the file name of the data file or the value of the \code{dataDscrpt}
option followed by the underscore separated concatenation of the \code{method}
and \code{fom} as the output file name. Since the Wilcoxon
statistic only applies to ROC data, the fourth example generates an
error. 

Alternatively, one can read the data file directly and skip
the dataset object creation step:
...
value is not a good figure of merit \citep{YoudenIndFor1950,HillisAComp2007}, as it does not give
...
This paper has covered several topics that are relevant
to assessment of medical imaging systems. These include the choice
of data collection paradigm (ROC, FROC or LROC), the choice of figure
of merit, significance-testing methods (DBM and ORH), and sample-size
estimation for ROC studies. Data visualization methods have been described
that could benefit from better curve-fitting algorithms. In our experience
statisticians tend to favor the empirical figures of merit, as these
are least based on what would be considered restrictive assumptions.
However, when the operating points do not span the ROC space adequately,
then empirical figures of merit become very dependent on the locations
of the points, which can lead to misleading inferences.
...
since the paradigm is very familiar, dating to the early 1940s (it
was originally introduced \citep{HildenTheArea1991} to measure performance of radar
in detecting enemy aircraft). The other figures of merit are less
well understood. Until 2004 \citep{ChaBerObser2004} there was no well-established way
of analyzing FROC data, but attempts began in the late 70s \citep{BunchHamAFree1978}
and some progress was made in the late 80s and early 90s \citep{ChaBreDigital1986,ChakraMax1989,ChaWinFree1990}.
The only way to assign a realistic effect size to an FROC figure of
merit is to have a model for fitting FROC data that also predicts
ROC data. As we have seen, the JAFROC effect size tends to be larger
than the corresponding ROC effect size, see Tables~\ref{tab:jafrocdbm} and~\ref{tab:inrocdbm}. To determine
the connection one needs a way of estimating the parameters of a model
of visual search that explains free-response and other operating characteristics.
Such a model has been introduced \citep{ChakraASear2006,ChakraROC2006} and a preliminary maximum
likelihood estimation method is implemented in the Windows version
of the software. We are currently working on enhancements to the estimation
procedure for improved reliability. The software currently does not
implement any of the ROC curve-fitting methods that are implemented
in the University of Iowa and University of Chicago website software.
Another direction for improvement is accommodating the LROC paradigm,
currently unsupported by any easily accessible software. Since \proglang{R}
is an open source platform it is our hope that this software will
lead others with interest in this field to contribute to it.
...
\subsection[{Other free-response figures of merit implemented in RJafroc}]{Other free-response figures of merit implemented in \pkg{RJafroc}}
...
Here $\phi\left(x\right)=1$ is $x$ is finite and $\phi\left({-\infty}\right)=0.$ The maximum NLF figure of merit is defined by
...
An exponentially transformed specificity figure of merit \citep{PopescuNonparSing2011} is
defined by:
...
in the Roe and Metz paper, were used:
