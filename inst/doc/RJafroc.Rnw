...
The purpose of this work is to describe \pkg{RJafroc} Version 0.0.2,
...
to the \code{frocData} \code{dataset} object. Since the default FOM is wJAFROC,  explicit
...
the \code{fom} option. The first example analyzes the data using the wJAFROC1 FOM, which uses the highest rated NL mark on diseased cases and gives equal importance to all diseased cases. The second example uses the JAFROC FOM, which tends to give extra importance to cases with many lesions but ignores NL marks on diseased cases. The third example illustrates usage of the JAFROC1 FOM, which uses the highest rated NL mark on diseased cases and gives extra importance to cases with many lesions. \emph{The wJAFROC1 and JAFROC1 FOMs should only be used if the dataset contains no non-diseased cases.}
...
The next example is deliberately included as an example of erroneaus usage of the function. It will generate an error since the \code{fom} option set to \code{Wilcoxon} is incompatible with the FROC dataset (see Table~\ref{tab:terminology} for valid FOMs with FROC data). 
